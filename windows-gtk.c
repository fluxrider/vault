...
// gcc --pedantic -Wall -Werror-implicit-function-declaration windows-gtk.c $(pkg-config --cflags --libs gtk4 libsodium)
//
// msys2
...
static const char * decrypt_init(const char * path, size_t * out_encrypted_n, int * out_fd, uint64_t * out_algo, uint64_t * out_algo_p1, uint64_t * out_algo_p2, unsigned char out_salt[crypto_pwhash_SALTBYTES], unsigned char out_nonce[crypto_aead_xchacha20poly1305_ietf_NPUBBYTES]) {
  ...
  // read header
  int fd = *out_fd = open(path, O_RDONLY | O_BINARY); if(fd == -1) error = "open()";
  uint64_t encrypted_n;
  if(!error) if(read(fd, out_algo, sizeof(uint64_t)) != sizeof(uint64_t)) error = "read()1";
  if(!error) if(read(fd, out_algo_p1, sizeof(uint64_t)) != sizeof(uint64_t)) error = "read()2";
  if(!error) if(read(fd, out_algo_p2, sizeof(uint64_t)) != sizeof(uint64_t)) error = "read()3";
  if(!error) if(read(fd, out_salt, crypto_pwhash_SALTBYTES) != crypto_pwhash_SALTBYTES) error = "read()4";
  if(!error) if(read(fd, out_nonce, crypto_aead_xchacha20poly1305_ietf_NPUBBYTES) != crypto_aead_xchacha20poly1305_ietf_NPUBBYTES) error = "read()5";
  if(!error) if(read(fd, &encrypted_n, sizeof(uint64_t)) != sizeof(uint64_t)) error = "read()6";
  if(!error){
...
static const char * decrypt(int fd, unsigned char * encrypted_buffer, unsigned char * decrypted_buffer, size_t * out_decrypted_n, size_t encrypted_n, uint64_t algo, uint64_t algo_p1, uint64_t algo_p2, unsigned char salt[crypto_pwhash_SALTBYTES], unsigned char nonce[crypto_aead_xchacha20poly1305_ietf_NPUBBYTES], const char * master_passphrase) {
  ...
  unsigned char key[crypto_aead_xchacha20poly1305_ietf_KEYBYTES] = {0}; {
    // read encrypted data
    ssize_t n = read(fd, encrypted_buffer, encrypted_n); if(n != encrypted_n) error = "read()7"; else { if(close(fd) == -1) error = "close()"; else {
      // derive key
...
static void on_file_with_passphrase(GtkDialog * dialog, gint response_id, gpointer _data) { GtkWidget ** _widgets = _data; GtkWidget * _entry = _widgets[0]; GtkWidget * _url = _widgets[1]; GtkWidget * _username = _widgets[2]; GtkWidget * _password = _widgets[3]; GtkWidget * _notes = _widgets[4]; GtkWidget * _master_passphrase = _widgets[6]; GFile * file = (GFile *) _widgets[7];
  ...
  if(!error) {
    ...
    unsigned char decrypted[encrypted_n - crypto_aead_xchacha20poly1305_ietf_ABYTES + 1];
    if(!error) {
      ...
      explicit_bzero(decrypted, encrypted_n - crypto_aead_xchacha20poly1305_ietf_ABYTES + 1);
    }
...
static void on_save_with_passphrase(GtkDialog * dialog, gint response_id, gpointer _data) { GtkWidget ** _widgets = _data; GtkWidget * _entry = _widgets[0]; GtkWidget * _url = _widgets[1]; GtkWidget * _username = _widgets[2]; GtkWidget * _password = _widgets[3]; GtkWidget * _notes = _widgets[4]; GtkWidget * _master_passphrase = _widgets[6];
  ...
  char * path = NULL;
  WIN32_FIND_DATA FindFileData; HANDLE hFind = FindFirstFile("*.enc", &FindFileData);
  if(hFind == INVALID_HANDLE_VALUE) {
    ...
    path = strdup(FindFileData.cFileName);
    FindClose(hFind);
  }
  if(!error) {
    ...
    if(!error) {
      ...
      if(!error) {
        ...
        explicit_bzero(decrypted, decrypted_n);
      }
    } else { if(fd != -1) close(fd); }
  }
  ...
  size_t src_n = entry_len+1+url_len+1+username_len+1+password_len+1+notes_len; char src[src_n]; {
  char * ptr = src;
  ...
  explicit_bzero(src, src_n); explicit_bzero(key, crypto_aead_xchacha20poly1305_ietf_KEYBYTES); {
  ...
  char filename[1024]; int n = snprintf(filename, 1024, "%s.%04d-%02d-%02d_%02d-%02d-%02d.enc", sanitized_entry, t->tm_year+1900, t->tm_mon+1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec); if(n > 1024 || n == -1) error = "snprintf()"; else {
  int fd = open(filename, O_CREAT | O_EXCL | O_WRONLY | O_BINARY ); if(fd == -1) error = "open()"; else {
  if(!error) if(write(fd, &algo, sizeof(uint64_t)) == -1) error = "writev()";
  if(!error) if(write(fd, &algo_p1, sizeof(uint64_t)) == -1) error = "writev()";
  if(!error) if(write(fd, &algo_p2, sizeof(uint64_t)) == -1) error = "writev()";
  if(!error) if(write(fd, salt, crypto_pwhash_SALTBYTES) == -1) error = "writev()";
  if(!error) if(write(fd, nonce, crypto_aead_xchacha20poly1305_ietf_NPUBBYTES) == -1) error = "writev()";
  if(!error) if(write(fd, &encrypted_n, sizeof(uint64_t)) == -1) error = "writev()";
  if(!error) if(write(fd, dst, encrypted_n) == -1) error = "writev()";
  if(!error) {
